...
interface QuickStatsProps {
  ...
  compositions?: BodyComposition[]; // Make optional
  goals?: BodybuildingGoal[]; // Make optional
  weightGoals?: WeightGoal[]; // Make optional
}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <div
                className="text-center p-3 rounded-lg bg-purple-50"
                aria-label="Body scans with body fat percentage"
              >
                <BarChart3 className="h-6 w-6 text-purple-600 mx-auto mb-1" />
                <p className="text-lg font-semibold text-purple-900">
                  {
                    compositions.filter((c) => c.bodyFatPercentage != null)
                      .length
                  }
                </p>
          ...
          <div className="text-center p-3 rounded-lg bg-green-50">
            ...
            <p className="text-lg font-semibold text-green-900">
              {weightGoals.filter((g) => g.isActive).length}
            </p>
          ...
          <div className="text-center p-3 rounded-lg bg-orange-50">
            ...
            <p className="text-lg font-semibold text-orange-900">
              {weights.length > 0
                ? Math.abs(
                    differenceInDays(new Date(), parseISO(weights[0].date))
                  )
                : 0}
            </p>
