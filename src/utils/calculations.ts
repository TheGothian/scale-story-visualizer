...
export const calculateTrend = (weights: WeightEntry[]): TrendData => {
  ...
  const changes = dataPoints.slice(1).map((point, i) => point.y - dataPoints[i].y);
  ...
  const movingAverage7 = calculateMovingAverage(dataPoints.map(p => p.y), 7);
  const movingAverage30 = calculateMovingAverage(dataPoints.map(p => p.y), 30);
...
const calculateLongestStreak = (weights: number[]) => {
  ...
  } else if (currentStreak.count === longestStreak.count && currentStreak.type === longestStreak.type) {
...
const calculateDayOfWeekPattern = (weights: WeightEntry[]) => {
  ...
  Object.entries(dayPattern).forEach(([day, weights]) => {
    averagePattern[day] = weights.length > 0 
